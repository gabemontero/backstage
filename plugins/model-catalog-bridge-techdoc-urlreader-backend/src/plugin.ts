...
// GGM temporary placeholder until we move to redhat-developer/rhdh-plugins/workspaces/ai-integrations and can import the version there
...
export function readModelCatalogApiEntityConfig(
  id: string,
  config: Config,
): ModelCatalogConfig {
  ...
  implements UrlReaderServiceReadTreeResponse
{
  ...
  async dir(
    ...
    try {
      ...
      await fs.writeFile(filePath1, buf);
    } catch (error) {
...
export class ModeCatalogBridgeTechdocUrlReader implements UrlReaderService {
  ...
  async readUrl(
    ...
    this.logger.info(`ModeCatalogBridgeTechdocUrlReader.readUrl of ${url}`);
  ...
  async search(
    ...
    this.logger.info(`ModeCatalogBridgeTechdocUrlReader.search of ${url}`);
  ...
  async readTree(
    ...
    this.logger.info(`ModeCatalogBridgeTechdocUrlReader.readTree of ${url}`);
    const data = await this.readUrl(url, options);
...
export const modelCatalogBridgeTechdocUrlreaderPlugin = createServiceFactory({
  ...
  async factory({ logger }) {
    logger
      .child({ source: 'modelCatalogBridgeTechdocUrlreaderPlugin"' })
