    ...
    const config = new ConfigReader({
      catalog: {
        providers: {
          modelCatalog: {
            provider1: {
              baseUrl: 'http://provider1.com:8080',
            },
            provider2: {
              baseUrl: 'http://provider2.com:9000',
            },
          },
        },
      },
...
describe('ModeCatalogBridgeTechdocUrlReader', () => {
  ...
  describe('constructor', () => {
    it('should read bridge configs', () => {
      ...
      // @ts-ignore
      expect(reader.bridgeConfigs[0].baseUrl).toBe('http://test.com:8080');
    });

    it('should use backend workingDirectory from config', () => {
      const reader = newReader({ backend: { workingDirectory: '/tmp/test' } });
      // @ts-ignore
      expect(reader.workDir).toBe('/tmp/test');
    });
  });
  ...
  describe('bridgePredicate', () => {
    it('should match default localhost URL', () => {
      const reader = newReader({});
      expect(
        reader.bridgePredicate(new URL('http://localhost:9090/modelcard/test')),
      ).toBe(true);
    });
    ...
    it('should match configured bridge URL', () => {
      ...
      expect(
        reader.bridgePredicate(new URL('http://test.com:8080/modelcard/test')),
      ).toBe(true);
    });
    ...
    it('should not match incorrect URL', () => {
      const reader = newReader({});
      expect(
        reader.bridgePredicate(new URL('http://other.com:9090/modelcard')),
      ).toBe(false);
    });
  });
  ...
  describe('search', () => {
    it('should return files for a successful search', async () => {
      ...
      const result = await reader.search('http://localhost:9090/modelcard');
      expect(result.files).toHaveLength(1);
      expect(result.files[0].url).toBe('http://localhost:9090/modelcard');
    ...
    it('should re-throw other errors', async () => {
      ...
      await expect(
        reader.search('http://localhost:9090/modelcard'),
...
describe('ModelCatalogBridgeUrlReaderServiceReadTreeResponse', () => {
  ...
  describe('dir', () => {
    it('should create temp dir and write files', async () => {
      const tempDir = '/tmp/backstage-123';
      ...
      expect(fs.mkdtemp).toHaveBeenCalledWith(path.join(workDir, 'backstage-'));
      expect(fs.writeFile).toHaveBeenCalledWith(
        path.join(tempDir, 'index.md'),
        await buffer,
      );
      expect(fs.mkdirSync).toHaveBeenCalledWith(path.join(tempDir, 'docs'));
      expect(fs.copyFileSync).toHaveBeenCalledWith(
        path.join(tempDir, 'index.md'),
        path.join(tempDir, 'docs', 'index.md'),
      );
      expect(resultDir).toBe(tempDir);
    });
